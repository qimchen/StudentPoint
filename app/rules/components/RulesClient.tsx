...
const subjectStyles: Record<string, { gradientClass: string; bgClass: string; borderClass: string }> = {
  '语文': { gradientClass: 'from-blue-400 to-blue-600', bgClass: 'bg-blue-50', borderClass: 'border-blue-200' },
  '数学': { gradientClass: 'from-green-400 to-green-600', bgClass: 'bg-green-50', borderClass: 'border-green-200' },
  '英语': { gradientClass: 'from-amber-400 to-amber-600', bgClass: 'bg-amber-50', borderClass: 'border-amber-200' },
};

const studentStyles = [
  { bgClass: 'bg-pink-50', borderClass: 'border-pink-200', gradientClass: 'from-pink-400 to-pink-600', textClass: 'text-pink-600' },
  { bgClass: 'bg-blue-50', borderClass: 'border-blue-200', gradientClass: 'from-blue-400 to-blue-600', textClass: 'text-blue-600' },
];
...
export default function RulesClient({ students, scoreItems }: RulesClientProps) {
  const groupedItems = subjects.map((subject) => ({
    subject,
    items: scoreItems.filter((item) => item.subject === subject),
  }));
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {groupedItems.map(({ subject, items }) => {
            const style = subjectStyles[subject] || subjectStyles['语文'];
            return (
              <div key={subject} className={`${style.bgClass} rounded-xl p-4 border ${style.borderClass}`}>
                <div className="flex items-center gap-2 mb-4">
                  <div className={`w-10 h-10 bg-gradient-to-br ${style.gradientClass} rounded-lg flex items-center justify-center text-white`}>
                    {renderIcon(subject === '语文' ? 'book' : subject === '数学' ? 'calculator' : 'language', 'w-5 h-5')}
                  </div>
                  <h3 className="font-bold text-lg">{subject}</h3>
                  <span className="text-xs text-gray-500">({items.length}项)</span>
                </div>
                {items.length === 0 ? (
                  <p className="text-sm text-gray-500 text-center py-4">暂无积分项</p>
                ) : (
                  <ul className="space-y-2">
                    {items.map((item) => (
                      <li key={item.id} className="flex justify-between items-center text-sm bg-white/60 rounded-lg px-3 py-2">
                        <span className="flex items-center gap-2">
                          <span className={iconColors[subject] || 'text-blue-600'}>{renderIcon('star', 'w-4 h-4')}</span>
                          <span className="text-gray-700">{item.name}</span>
                        </span>
                        <span className={`font-bold ${iconColors[subject] || 'text-blue-600'}`}>+{item.points}</span>
                      </li>
                    ))}
                  </ul>
                )}
          ...
          {students.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {students.map((student, index) => {
                const style = studentStyles[index % studentStyles.length];
                return (
                  <div key={student.id} className={`${style.bgClass} rounded-xl p-4 border ${style.borderClass}`}>
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-12 h-12 bg-gradient-to-br ${style.gradientClass} rounded-xl flex items-center justify-center`}>
                        {renderIcon('user', 'w-6 h-6 text-white')}
                      </div>
                      <div>
                    ...
                    <div className="bg-white rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`font-bold ${style.textClass}`}>{student.exchangeRate}%</span>
                      </div>
                      <div className="progress-bar">
                        <div 
                          className={`progress-bar-fill bg-gradient-to-r ${style.gradientClass}`}
                          style={{ width: `${student.exchangeRate}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  当兑换积分≥100时，需要确保各科目积分均衡发展。例如{students.map(s => `${s.name}需≥${s.exchangeRate}%`).join('，')}。
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  这样可以鼓励小朋友全面发展，不偏科。
                </p>
